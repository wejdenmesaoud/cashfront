...
export class AuthUtils
{
    ...
    {
        console.log('Checking if token is expired:', token ? token.substring(0, 20) + '...' : token);
        ...
        try {
            ...
            if ( date === null )
            {
                console.log('No expiration date in token, considering expired');
                return true;
            }

            const now = new Date();
            const isExpired = !(date.valueOf() > now.valueOf() + offsetSeconds * 1000);
    ...
    {
        console.log('Decoding token:', token ? token.substring(0, 20) + '...' : token);
        ...
        try {
            ...
            if ( parts.length !== 3 )
            {
                console.error('Token does not have 3 parts:', parts);
